...
const isArtsMarksheet =
  (
    extractedText.match(
      /\b(history|political\s+science|geography|sociology|psychology|philosophy|home\s+science|fine\s+arts|music|hindi|english)\b/gi
    ) || []
  ).length >= 2;


    return { text: extractedText, isArtsMarksheet: isArtsMarksheet };
  } catch (error) {
